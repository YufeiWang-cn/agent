...
复杂任务可以通过 `update_plan` 维护 2 到 7 个结构化步骤。单轮模型执行步数默认上限为 12，可以通过 `AGENT_MAX_STEPS` 调整；达到步数上限或模型结束时仍有未结束步骤，任务都会以“未完成”状态停止，而不会被误报为成功。
...
多步骤任务会在对话区顶部显示可折叠计划卡片，实时展示待处理、进行中、已完成、失败和已跳过状态。最近一次计划会随会话保存并在重新打开后恢复；简单问答不会强制生成计划。
